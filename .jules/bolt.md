...
## 2026-07-20 - Limit MySQL found row computation on single record fetches
**Learning:** The `Modul` base class in `Janmensik\Jmlib\Modul` prefixes standard data queries with `SQL_CALC_FOUND_ROWS` (`$sql_base`). This is slow and deprecated in MySQL 8, especially when we only need a single row from methods like `getAdForDevice` via `LIMIT 1` or `LIMIT 20` where the total row count across the entire table is ignored and unused.
**Action:** When querying small limits without pagination requirements using `Modul::get()`, pass `true` as the 5th parameter (`$nocalcrows = true`) to explicitly disable the `SQL_CALC_FOUND_ROWS` overhead on that specific execution, vastly reducing the database scan depth and latency while leaving the default pagination abilities untouched.
