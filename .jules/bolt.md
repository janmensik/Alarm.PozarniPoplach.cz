...
## 2024-07-17 - Avoid `SQL_CALC_FOUND_ROWS` on Non-Paginated Queries
**Learning:** `Janmensik\Jmlib\Modul` queries typically include `SQL_CALC_FOUND_ROWS` in their `$sql_base` for pagination. This runs regardless of whether the query actually needs the count (e.g., getting a single random Ad or Ad by ID). `SQL_CALC_FOUND_ROWS` is deprecated in MySQL 8 and is an unnecessary performance penalty when fetching small, bounded result sets.
**Action:** When calling `$this->get()` in `Janmensik\Jmlib\Modul` subclasses (like `class.Ad.php` and `class.Unit.php`) for queries that do not require pagination (like fetching with `LIMIT 1` or `LIMIT 20` for random selection), use `$this->getNoCalcRows()` or pass `true` as the 5th parameter to `get()` to skip the expensive `SQL_CALC_FOUND_ROWS` execution. Note that methods fetching Ads are duplicated across `class.Ad.php` and `class.Unit.php`.
