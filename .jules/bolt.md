...
## 2026-07-05 - Defer Heavy Data Fetch During Peacetime Polling
**Learning:** High-frequency polling endpoints like `/api/dispatch` were unconditionally fetching heavy, relational data (e.g., vehicles via `getDispatch()`) on every request, even during "peacetime" when no alarm was active and this data was discarded by the frontend. This created significant, unnecessary database read load (multiple JOINs and supplementary queries) every 30 seconds per device.
**Action:** When an endpoint frequently polls for state changes (like waiting for an alarm), only fetch the base state required to evaluate the condition initially. Use an optional flag (e.g., `$full_data = false`) to skip the heavier relational lookups. If the condition is met (an alarm is active), then execute the heavy data fetch before proceeding.

## 2024-07-20 - Prevent SQL_CALC_FOUND_ROWS overhead on single-record fetches
**Learning:** Using `$this->get()` in `Janmensik\Jmlib\Modul` inheriting classes inherently injects the `SQL_CALC_FOUND_ROWS` directive for pagination purposes. When fetching a small, fixed limit of records (like fetching 1 or 20 ads) where pagination metadata is not required, this calculation forces MySQL to scan all matching rows, creating an unnecessary performance bottleneck on large tables.
**Action:** When pagination is not required, use `$this->getNoCalcRows()` instead of `$this->get()` to dynamically prevent the expensive `SQL_CALC_FOUND_ROWS` overhead without needing to modify the base query string. This is particularly impactful for operations like fetching random ads or specific IDs. Note: Do not use `getNoCalcRows` for queries feeding into `getRandom()`, as it relies on `$this->cache_total` which requires the row calculation.
