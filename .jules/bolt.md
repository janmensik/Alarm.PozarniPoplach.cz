...
## 2024-07-06 - Avoid Heavy Inherited Queries on High-Frequency Endpoints
**Learning:** The `Ad::getAdData()` method was relying on the inherited `Modul::get()` method to fetch a single ad by ID. However, `Ad::$sql_base` includes `SQL_CALC_FOUND_ROWS`, `LEFT JOIN` on the `advert_hit` table, and `SUM()/GROUP BY` aggregations. This caused significant, unnecessary database load because this method is called every 30 seconds by all devices during peacetime.
**Action:** When fetching a single record for a high-frequency endpoint, inspect the parent class's `$sql_base`. If it contains heavy aggregations or joins that aren't needed for the specific context, bypass the generic `get()` method and write a targeted, direct SQL query that only selects the required fields.
