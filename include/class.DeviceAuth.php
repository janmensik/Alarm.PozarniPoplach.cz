...
class DeviceAuth extends Modul
{
     ...
     */
    /**
     * @var array Cache for device records to avoid redundant queries in other modules during the same request cycle.
     */
    private array $cachedDeviceRecords = [];

    /**
     * Retrieves cached data for a specific device.
     */
    public function getCachedDevice(string $deviceUuid): ?array
    {
        return $this->cachedDeviceRecords[$deviceUuid] ?? null;
    }

    public function validateDevice(string $deviceUuid, string $refreshToken): int|null
    {
        // Optimization: Fetch all necessary columns in the initial query and cache the row.
        // This prevents redundant DB hits in Ad/Unit modules for the same device.
        ...
        if ($device && hash_equals($device['refresh_token_hash'], hash('sha256', $refreshToken))) {
            $this->cachedDeviceRecords[$deviceUuid] = $device;
