...
{
    /**
     * @var array|null Caches the full device row during validation to prevent duplicate queries.
     */
    public ?array $currentDeviceRow = null;
    ...
    public function validateDevice(string $deviceUuid, string $refreshToken): int|null
    {
        ...
        if ($device && hash_equals($device['refresh_token_hash'], hash('sha256', $refreshToken))) {
            // Cache the device row to prevent redundant queries in other modules (like Ad logic)
            $this->currentDeviceRow = $device;
