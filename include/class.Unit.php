...
class Unit extends Modul
{
    ...
    public function getAdForDevice(string $deviceUuid, int $unitId): array|null
    {
        ...
        if ($roll <= $device['ad_probability']) {
            ...
            // Optimization: Used lightweight direct SQL query instead of getNoCalcRows
            // to prevent hydrating full data objects for 20 ads when we only need to pick 1 random ID
            $ads = $this->DB->getAllRows($this->DB->query('SELECT id FROM advert WHERE status = "active" LIMIT 20'));
            if (!empty($ads)) {
