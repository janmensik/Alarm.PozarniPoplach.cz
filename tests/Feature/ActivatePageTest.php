...
test('activate page accepts POST with valid CSRF token', function () {
    ...
    // checkUnitPincode calls getResult
...
test('activate page rejects POST with incorrect pincode', function () {
    $_SERVER['REQUEST_METHOD'] = 'POST';
    $_SESSION['csrf_token'] = 'valid-token';
    $_POST = [
        'unit_id' => 123,
    ...
    // getRow gets called in checkSessionStatus
    $this->db->expects($this->any())->method('getRow')->willReturn([
        'status' => 'pending',
        'unit_id' => null,
        'device_uuid' => 'test-uuid'
    ...
    // checkUnitPincode calls getResult and returns null for invalid pincode
