...
test('it prevents SSRF by blocking local IPs', function () {
    $calendar = new Calendar('http://127.0.0.1/test.ics');
    expect($calendar->calendar_url)->toBe('');

    $calendar2 = new Calendar('http://localhost/test.ics');
    expect($calendar2->calendar_url)->toBe('');

    $calendar3 = new Calendar('http://169.254.169.254/latest/meta-data/');
    expect($calendar3->calendar_url)->toBe('');
});
