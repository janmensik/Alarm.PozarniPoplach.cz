...
if ($session && !empty($_POST['unit_id'])) {
    ...
    $unit_data = $DB->getRow($DB->query('SELECT pincode FROM unit WHERE id = ' . $unit_id));

    if (empty($_POST['pincode']) || !$unit_data || !hash_equals((string)$unit_data['pincode'], (string)$_POST['pincode'])) {
